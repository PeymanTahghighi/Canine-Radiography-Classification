...
from deep_learning.UNETR import UNETRC
...
def extract_symmetry_features(img_left, img_right, spine):
    ...
    #cv2.imwrite(f'tmp\\{t}.png', ret_mask_left);
    #right_thorax = cv2.cvtColor(right_thorax, cv2.COLOR_GRAY2RGB);
    #right_thorax = cv2.circle(right_thorax, (top_right[0], top_right[1]), 5, (255,0,0,), -1);
    dist_left = 0;
    x_left = top_left[0];
    x_right = top_right[0];
...
def tmp():
    ...
    ribs_mask = cv2.imread(f'C:\\Users\\Admin\\OneDrive - University of Guelph\\Miscellaneous\\Spine and Ribs\\labels\\DV13_1.png', cv2.IMREAD_GRAYSCALE);
    ...
    cv2.imwrite(f'fig7_left.png', thorax_left);
    cv2.imwrite(f'fig7_right.png', thorax_right);
    cv2.imwrite(f'fig7_left_t.png', ribs_left);
    cv2.imwrite(f'fig7_right_t.png', ribs_right);
...
#         test_x, test_mask, test_exp_lbl);
   ...
   # compare_thorax_segmentation();
    ...
    spine_and_ribs_segmentation_model = UNETRC(1,config.IMAGE_SIZE,16,768,12,0.0,4,3).to(config.DEVICE);

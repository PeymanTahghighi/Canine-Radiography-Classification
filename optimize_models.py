...
def optimize_sternum_model(folds, rebuild_features = False):
    best_class_thresh = 0;
    best_pix_thresh = 0;
    ...
    if rebuild_features:
        for fold_idx, f in enumerate(folds):
            ...
            for idx,t in tqdm(enumerate(train_imgs)):
                index = img_list.index(t);
                sternum_mask = pickle.load(open(train_mask[idx][2],'rb'));
                thorax_mask = cv2.imread(f'C:\\PhD\\Thesis\\Unsupervised Canine Radiography Classification\\results\\train_data\\{t}.png', cv2.IMREAD_GRAYSCALE);
                
                kernel = np.array([[1,0,1],[1,0,1],[1,0,1]], dtype=np.uint8);
                thorax_mask = cv2.erode(thorax_mask, kernel, iterations=10);
                thorax_mask = cv2.resize(thorax_mask, (sternum_mask.shape[1], sternum_mask.shape[0]));
                ...
                sternum_mask = (sternum_mask>0) * (residual>0);
                fig, ax = plt.subplots(1,4);
                ax[0].imshow(thorax_mask);
                ax[1].imshow(spine_mask);
                ax[2].imshow(sternum_mask);
                ax[3].imshow(residual);
                fig.savefig(f'tmp\\{t}_train.png');
                plt.close('all');

                feat = np.sum(sternum_mask)/np.sum(thorax_mask>0);
                total_X.append(feat);
                lbl_in_list = sternum_list[index];
                total_Y.append(lbl_in_list);
            
            for idx,t in tqdm(enumerate(test_imgs)):
                index = img_list.index(t);
                sternum_mask = cv2.imread(f'results\\{fold_idx}\\outputs\\{t}_sternum.png', cv2.IMREAD_GRAYSCALE);
                thorax_mask = cv2.imread(f'C:\\PhD\\Thesis\\Unsupervised Canine Radiography Classification\\results\\train_data\\{t}.png', cv2.IMREAD_GRAYSCALE);
                ...
                spine_mask = cv2.resize(spine_mask, (sternum_mask.shape[1], sternum_mask.shape[0]));
        ...
        pickle.dump([total_X, total_Y, custom_cv], open('data_sternum.dmp', 'wb'));
    ...
    for train_id, test_id in custom_cv:
        ...
        for i,p in enumerate(pred):
            if p != test_y[i]:
                print(f'{test_x[i]}\tpred: {p}\tgt:{test_y[i]}');

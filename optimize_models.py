...
from sklearn.preprocessing import LabelEncoder, OneHotEncoder, RobustScaler, Normalizer, scale, StandardScaler
...
from utils import create_folder, extract_symmetry_features
...
def optimize_sternum_model(folds):
    ...
    for thresh in thresholds:
        ...
        for idx,f in enumerate(folds):
            ...
            for index,t in tqdm(enumerate(test_imgs)):
                if test_grain_lbl[index][3] == 'Yes' or  test_grain_lbl[index][3] == 'Mid' or  test_grain_lbl[index][3] == 'No':
                    sternum_feat = pickle.load(open(f'results\\{idx}\\outputs\\{t}_sternum.feat','rb'));
        ...
        total_x = np.expand_dims(total_x[:,1], axis = 1);
        #h,w = total_x.shape;
        total_y[total_y=='No'] = 0;
        total_y[total_y=='Mid'] = 0;
...
def optimize_symmetry_model(folds):
    total_X = [];
    total_Y = [];
    train_fold_indices = [];
    test_fold_indices = [];
    ...
    for idx,f in enumerate(folds):
        ...
        train_fold_indices.append(np.arange(len(total_X),len(total_X)+len(train_imgs)));
        test_fold_indices.append(np.arange(len(total_X)+len(train_imgs),len(total_X)+len(train_imgs)+len(test_imgs)));

        

        #train
        for i in tqdm(range(len(train_imgs))):    
            lbl = train_grain_lbl[i][2];
            if lbl =='0' or lbl == '1':
                lbl = 0;
            else:
                lbl = 1;
            ...
            # left = cv2.imread(f'results\\train_data\\{train_imgs[i]}_left.png', cv2.IMREAD_GRAYSCALE);
            # right = cv2.imread(f'results\\train_data\\{train_imgs[i]}_right.png', cv2.IMREAD_GRAYSCALE);
            # symmetry_features = extract_symmetry_features(left, right);
            total_X.append(total_x[i]);
    ...
    best_idx = -1;
    best_score = 0;
    com = list(range(0,len(ps)));
    random.shuffle(com);
    for i in tqdm(range(0, len(com))):
        if len(ps[com[i]]) > 1:
            features = np.zeros((total_x.shape[0], len(ps[com[i]])));
            for j in range(len(ps[com[i]])):
                features[:,j] = total_x[:,ps[com[i]][j]];
